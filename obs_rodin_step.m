...
% Input matrices for observer model
B_obs = B(:, u_meas);
D_obs = D(:, u_meas);
...
poles = [0.8; 0.8];
LB1 = luenberger_filter(A,B_obs,C,D_obs,Ts,poles,'LB1');

% Steady-state Luenberger observer 2
% Specify poles of observer dynamics
poles = [0.6; 0.6];
LB2 = luenberger_filter(A,B_obs,C,D_obs,Ts,poles,'LB2');
...
R = sigma_M^2;
KFSS1 = kalman_filter_ss(A,B_obs,C,D_obs,Ts,Q,R,'KFSS1');

% Steady-state Kalman filter 2 - tuned to sigma_wp(2)
Q = diag([Q1 sigma_wp(2)^2]);
R = sigma_M^2;
KFSS2 = kalman_filter_ss(A,B_obs,C,D_obs,Ts,Q,R,'KFSS2');
...
R = sigma_M^2;
KF1 = kalman_filter(A,B_obs,C,D_obs,Ts,P0,Q,R,'KF1');
...
R = sigma_M^2;
KF2 = kalman_filter(A,B_obs,C,D_obs,Ts,P0,Q,R,'KF2');
...
Q = diag([Q1 adj*0.1^2]);
R = sigma_M^2;
KF3 = kalman_filter(A,B_obs,C,D_obs,Ts,P0,Q,R,'KF3');

% Scheduled Kalman filter
P0 = 1000*eye(n);
Q0 = diag([Q1 1]);
R = sigma_M^2;
SKF = kalman_filter(A,B_obs,C,D_obs,Ts,P0,Q0,R,'SKF');
SKF.Q0 = Q0;
