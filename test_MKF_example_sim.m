...
% TODO: Remove this when old KF objects deleted
observers = {KF_old};
...
assert(max(abs(Xk_est_old(1:end-1,:) - ...
    sim_results{2:end, {'Xk_est_KFP_1', 'Xk_est_KFP_2'}}), [], [1 2]) < 1e-12)

% Check estimates of old KF struct same as KFP here
assert(max(abs(sim_out.X_hat_KFP.Data - [0 0; Xk_est_old(1:end-1,:)]), [], [1 2]) < 1e-15)
